...
export default class LegacyServerListPingPacket extends Packet {
    ...
    public decrypt() {
        ...
        this.PingHost = this.getBytes()
            .slice(this.getOffset(), this.addOffset(this.PingHostLength * 2, true))
            .filter(v => v !== 0)
            .toString();
        this.RestData = this.readShort();
        this.ProtocolVersion = this.readUnsignedByte();
        this.HostnameLength = this.readShort();
        this.Hostname = this.getBytes()
            .slice(this.getOffset(), this.addOffset(this.HostnameLength * 2, true))
            .filter(v => v !== 0)
            .toString();
